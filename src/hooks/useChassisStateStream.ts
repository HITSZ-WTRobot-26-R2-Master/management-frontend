...
export type ChassisStateStreamStatus =
  | "auth_required"
  | "connecting"
  | "live"
  | "fallback"
...
export interface ChassisStateStreamState {
  ...
  status: ChassisStateStreamStatus
}

export function useChassisStateStream(): ChassisStateStreamState {
  ...
  const [state, setState] = useState<ChassisStateStreamState>({
    error: null,
    lastMessageAt: null,
    refresh: () => undefined,
    snapshot: null,
  ...
  useEffect(() => {
    ...
    const refreshFallback = async () => {
      ...
      try {
        const snapshot = await client.getChassisState(controller.signal)
    ...
    try {
      socket = new WebSocket(buildChassisStateWebSocketUrl(baseUrl, token))
      setState((current) => ({
        ...current,
        error: null,
        status: "connecting",
    ...
    socket.addEventListener("message", (message) => {
      ...
      const parsed = parseChassisStateSocketMessage(message.data)
      if (!parsed) {
        return
      }

      if (parsed.type === "chassis_state_snapshot") {
        setState((current) => ({
          ...current,
          error: null,
...
function parseChassisStateSocketMessage(data: unknown) {
  ...
  try {
    ...
    return isChassisStateWebSocketMessage(parsed) ? parsed : null
  } catch {
    return null
  }
}

export function chassisStateConnectionStatus(
  status: ChassisStateStreamStatus,
): ConnectionStatus {
  switch (status) {
    case "auth_required":
