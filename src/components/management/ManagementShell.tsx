...
const navItems = [
  { label: "Services", active: true, icon: Boxes },
  { label: "Events", active: false, icon: Bell },
  { label: "Commands", active: false, icon: DatabaseZap },
    ...
    <div className="min-h-screen">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Operations plane for registered ROS2 services
            </p>
          </div>
          <nav aria-label="Management navigation" className="flex flex-wrap gap-2">
            {navItems.map((item) => (
              <button
                key={item.label}
                type="button"
      ...
      <footer className="border-t border-border bg-card/70">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-5 py-4 text-sm text-muted-foreground md:flex-row md:items-center md:justify-between">
          <span>Uses logical service names returned by the backend registry.</span>
          <span className="inline-flex items-center gap-2">
            <LifeBuoy aria-hidden="true" className="size-4" />
            Live events with REST fallback keep the dashboard current.
          </span>
        </div>
      </footer>
    </div>
